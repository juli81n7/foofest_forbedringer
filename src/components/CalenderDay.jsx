  ...
  if (props.personalProgram) {
    ...
    props.Jotunheim.map((thing) => {
      props.personalProgram.map((item) => {
        thing.act === item.name ? Jotunheim.push(thing) : null;
      });
    });
    props.Midgard.map((thing) => {
      props.personalProgram.map((item) => {
        thing.act === item.name ? Midgard.push(thing) : null;
      });
    });
    props.Vanaheim.map((thing) => {
      props.personalProgram.map((item) => {
        thing.act === item.name ? Vanaheim.push(thing) : null;
      });
    });
  }
    ...
    <>
      <div id={props.day} className="calenderarea">
        ...
        <div className="calenderbar">
          ...
          {Jotunheim.map((thing) => (thing.act !== "break" ? <CalenderEvent scene="jotunheim" img2={props.bands.find((band) => band.name === thing.act).logo} slug={props.bands.find((band) => band.name === thing.act).slug} key={props.bands.find((band) => band.name === thing.act).id} name={thing.act} start={thing.start} end={thing.end} /> : null))}
        </div>
        <div className="calenderbar">
          <div className="calenderhour"></div>
          <div className="calenderhour"></div>
        ...
        <div className="calenderbar">
          ...
          {Vanaheim.map((thing) => (thing.act !== "break" ? <CalenderEvent scene="vanaheim" img2={props.bands.find((band) => band.name === thing.act).logo} slug={props.bands.find((band) => band.name === thing.act).slug} key={props.bands.find((band) => band.name === thing.act).id} name={thing.act} start={thing.start} end={thing.end} /> : null))}
        </div>
      </div>
    </>
  );
